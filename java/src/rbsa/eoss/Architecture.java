...
public class Architecture implements Comparable<Architecture>, java.io.Serializable {
    private static final long serialVersionUID =  -4234661875933954997L;
    ...
    /*public Architecture(HashMap mapping, int nsat) {
        mat = new boolean[Params.norb][Params.ninstr];
         for(int o = 0;o<Params.norb;o++) {
             for(int i = 0;i<Params.ninstr;i++) {
                 mat[o][i] = false;
        ...
        for(int o = 0;o<Params.norb;o++) {
            String orb = Params.orbit_list[o];
            List<String> payl = (List)mapping.get(orb);
            if (payl == null)
                continue;
            ArrayList<String> thepayl = new ArrayList<String>();
            thepayl.addAll(payl);
    ...
    public boolean containsInst(String instr){
        ...
        for (int o = 0;o<norb;o++) {
            ...
            if(payls!=null) {
                ArrayList<String> paylds = new ArrayList();
                paylds.addAll(Arrays.asList(payls));
    ...
    private void updateOrbitPayload() {
        //TODO fix the payload and payloads update
        // it resets after ever iteration of i
        for (int i = 0; i < Params.norb; i++) {
            int n = SumRowBool(mat, i);
            if (n > 0) {
    ...
    public Architecture mutate1bit() {
        ...
        do {
            if (rnd.nextBoolean()) { //mutate matrix but not nsats
                 ...
                 new_one = new Architecture(new_bitString,this.norb,this.ninstr,nsats);
                 new_one.setCrossover(crossover);
                 new_one.setImprove(improve);
                 //return new_one;
            } else {///mutate nsats but not matrix

                 new_one = new Architecture(bitString,this.norb,this.ninstr,1+rnd.nextInt(Params.nsats.length-1));
                 new_one.setCrossover(crossover);
                 new_one.setImprove(improve);
                 //return new_one;
            }
    ...
    public Architecture crossover1point(Architecture other) {
        ...
        do {
            ...
            if (rnd.nextBoolean())
                ...
                new_one = new Architecture(new_bitString,this.norb,this.ninstr,other.getNsats());
        } while(!new_one.checkConstraints());
        return new_one;
    }
    ...
    public boolean checkConstraints() {
        ...
        for (String orb:Params.orbit_list) {
            String[] payload_in_orbit = this.getPayloadInOrbit(orb);
            if (orb.equalsIgnoreCase("GEO-35788-equat-NA"))
                continue;
            for (String instr : payload_in_orbit) 
                if (instr.equalsIgnoreCase("PATH_GEOSTAR"))
                    return false;    
        }
        
        //Constraint 2: No LEO instruments in GEO Orbit
        String[] GEO_payload = this.getPayloadInOrbit("GEO-35788-equat-NA");
    ...
    public int hashCode() {
        int hash = 5;
        hash = 79 * hash + Arrays.hashCode(this.bitString);
        hash = 79 * hash + this.nsats;
        return hash;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!Arrays.equals(this.bitString, other.bitString)) {
            return false;
        }
